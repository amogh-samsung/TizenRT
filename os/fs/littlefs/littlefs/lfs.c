...
static int lfs_alloc(lfs_t *lfs, lfs_block_t *block) {
    while (true) {
        ...
        if (lfs->free.ack == 0) {
            fdbg("No more free space %"PRIu32, lfs->free.i + lfs->free.off);
...
static lfs_stag_t lfs_dir_fetchmatch(lfs_t *lfs,
    ...
    fdbg("Corrupted dir pair at {0x%"PRIx32", 0x%"PRIx32"}", dir->pair[0], dir->pair[1]);
    return LFS_ERR_CORRUPT;
}
    ...
    if (lfs->cfg->block_cycles > 0 &&
        ...
        if (lfs_pair_cmp(dir->pair, (const lfs_block_t[2]){0, 1}) == 0) {
            ...
            if ((lfs_size_t)res < lfs->cfg->block_count/2) {
                fdbg("Expanding superblock at rev %"PRIu32, dir->rev);
...
relocate:
        ...
        if (!tired) {
            fdbg("Bad block at 0x%"PRIx32, dir->pair[1]);
        }
    ...
    if (relocated) {
        ...
        fdbg("Relocating {0x%"PRIx32", 0x%"PRIx32"} "
                    "-> {0x%"PRIx32", 0x%"PRIx32"}",
...
relocate:
        fdbg("Bad block at 0x%"PRIx32, nblock);
...
relocate:
                fdbg("Bad block at 0x%"PRIx32, file->block);
...
static int lfs_rawmount(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    while (!lfs_pair_isnull(dir.tail)) {
        ...
        if (tag && !lfs_tag_isdelete(tag)) {
            ...
            if ((major_version != LFS_DISK_VERSION_MAJOR ||
                ...
                fdbg("Invalid version v%"PRIu16".%"PRIu16,
            ...
            if (superblock.name_max) {
                if (superblock.name_max > lfs->name_max) {
                    fdbg("Unsupported name_max (%"PRIu32" > %"PRIu32")",
            ...
            if (superblock.file_max) {
                if (superblock.file_max > lfs->file_max) {
                    fdbg("Unsupported file_max (%"PRIu32" > %"PRIu32")",
            ...
            if (superblock.attr_max) {
                if (superblock.attr_max > lfs->attr_max) {
                    fdbg("Unsupported attr_max (%"PRIu32" > %"PRIu32")", superblock.attr_max, lfs->attr_max);
                    err = LFS_ERR_INVAL;
                    goto cleanup;
                }
    ...
    if (!lfs_gstate_iszero(&lfs->gstate)) {
        fdbg("Found pending gstate 0x%08"PRIx32"%08"PRIx32"%08"PRIx32,
                lfs->gstate.tag,
                lfs->gstate.pair[0],
                lfs->gstate.pair[1]);
    }
...
static int lfs_fs_relocate(lfs_t *lfs,
    ...
    if (tag != LFS_ERR_NOENT) {
        ...
        if (lfs_gstate_hasmovehere(&lfs->gstate, parent.pair)) {
            ...
            fdbg("Fixing move while relocating "
...
static int lfs_fs_demove(lfs_t *lfs) {
    ...
    // Fix bad moves
    fdbg("Fixing move {0x%"PRIx32", 0x%"PRIx32"} 0x%"PRIx16,
            lfs->gdisk.pair[0],
            lfs->gdisk.pair[1],
...
static int lfs_fs_deorphan(lfs_t *lfs) {
    ...
    while (!lfs_pair_isnull(pdir.tail)) {
        ...
        if (!pdir.split) {
            ...
            if (tag == LFS_ERR_NOENT) {
                // we are an orphan
                fdbg("Fixing orphan {0x%"PRIx32", 0x%"PRIx32"}",
            ...
            if (!lfs_pair_sync(pair, pdir.tail)) {
                // we have desynced
                fdbg("Fixing half-orphan {0x%"PRIx32", 0x%"PRIx32"} "
                            "-> {0x%"PRIx32", 0x%"PRIx32"}",
...
static int lfs1_dir_fetch(lfs_t *lfs,
    ...
    if (!valid) {
        fdbg("Corrupted dir pair at {0x%"PRIx32", 0x%"PRIx32"}",
                tpair[0], tpair[1]);
        return LFS_ERR_CORRUPT;
    }
...
static int lfs1_mount(lfs_t *lfs, struct lfs1 *lfs1,
    ...
    int err = 0;
    {
        ...
        if (err || memcmp(superblock.d.magic, "littlefs", 8) != 0) {
            fdbg("Invalid superblock at {0x%"PRIx32", 0x%"PRIx32"}",
                    0, 1);
            err = LFS_ERR_CORRUPT;
            goto cleanup;
        }
        ...
        if ((major_version != LFS1_DISK_VERSION_MAJOR ||
            ...
            fdbg("Invalid version v%d.%d", major_version, minor_version);
            err = LFS_ERR_INVAL;
            goto cleanup;
        }
...
static int lfs_rawmigrate(lfs_t *lfs, const struct lfs_config *cfg) {
...

    {
        ...
        while (!lfs_pair_isnull(dir1.d.tail)) {
            ...
            fdbg("Migrating {0x%"PRIx32", 0x%"PRIx32"} "
                        "-> {0x%"PRIx32", 0x%"PRIx32"}",
...
int lfs_unmount(lfs_t *lfs) {
    ...
    fvdbg("lfs_unmount(%p)", (void*)lfs);

    err = lfs_rawunmount(lfs);

    fvdbg("lfs_unmount -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
...
int lfs_remove(lfs_t *lfs, const char *path) {
    ...
    fvdbg("lfs_remove(%p, \"%s\")", (void*)lfs, path);

    err = lfs_rawremove(lfs, path);

    fvdbg("lfs_remove -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
#endif
...
int lfs_rename(lfs_t *lfs, const char *oldpath, const char *newpath) {
    ...
    fvdbg("lfs_rename(%p, \"%s\", \"%s\")", (void*)lfs, oldpath, newpath);

    err = lfs_rawrename(lfs, oldpath, newpath);

    fvdbg("lfs_rename -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
#endif
...
int lfs_stat(lfs_t *lfs, const char *path, struct lfs_info *info) {
    ...
    fvdbg("lfs_stat(%p, \"%s\", %p)", (void*)lfs, path, (void*)info);

    err = lfs_rawstat(lfs, path, info);

    fvdbg("lfs_stat -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
...
int lfs_setattr(lfs_t *lfs, const char *path,
    ...
    fvdbg("lfs_setattr(%p, \"%s\", %"PRIu8", %p, %"PRIu32")",
            (void*)lfs, path, type, buffer, size);

    err = lfs_rawsetattr(lfs, path, type, buffer, size);

    fvdbg("lfs_setattr -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
#endif
...
int lfs_removeattr(lfs_t *lfs, const char *path, uint8_t type) {
    ...
    fvdbg("lfs_removeattr(%p, \"%s\", %"PRIu8")", (void*)lfs, path, type);

    err = lfs_rawremoveattr(lfs, path, type);

    fvdbg("lfs_removeattr -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
#endif
...
int lfs_file_open(lfs_t *lfs, lfs_file_t *file, const char *path, int flags) {
    ...
    fvdbg("lfs_file_open(%p, %p, \"%s\", %x)",
            (void*)lfs, (void*)file, path, flags);
    LFS_ASSERT(!lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file));

    err = lfs_file_rawopen(lfs, file, path, flags);

    fvdbg("lfs_file_open -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
...
int lfs_file_opencfg(lfs_t *lfs, lfs_file_t *file,
    ...
    fvdbg("lfs_file_opencfg(%p, %p, \"%s\", %x, %p {"
...
int lfs_file_close(lfs_t *lfs, lfs_file_t *file) {
    ...
    fvdbg("lfs_file_close(%p, %p)", (void*)lfs, (void*)file);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file));

    err = lfs_file_rawclose(lfs, file);

    fvdbg("lfs_file_close -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
...
int lfs_file_sync(lfs_t *lfs, lfs_file_t *file) {
    ...
    fvdbg("lfs_file_sync(%p, %p)", (void*)lfs, (void*)file);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file));

    err = lfs_file_rawsync(lfs, file);

    fvdbg("lfs_file_sync -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
#endif
...
lfs_ssize_t lfs_file_read(lfs_t *lfs, lfs_file_t *file,
    ...
    fvdbg("lfs_file_read(%p, %p, %p, %"PRIu32")",
            (void*)lfs, (void*)file, buffer, size);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file));

    lfs_ssize_t res = lfs_file_rawread(lfs, file, buffer, size);

    fvdbg("lfs_file_read -> %d", res);
    LFS_UNLOCK(lfs->cfg);
    return res;
}
...
lfs_ssize_t lfs_file_write(lfs_t *lfs, lfs_file_t *file,
    ...
    fvdbg("lfs_file_write(%p, %p, %p, %"PRIu32")",
            (void*)lfs, (void*)file, buffer, size);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file));

    lfs_ssize_t res = lfs_file_rawwrite(lfs, file, buffer, size);

    fvdbg("lfs_file_write -> %d", res);
    LFS_UNLOCK(lfs->cfg);
    return res;
}
#endif
...
lfs_soff_t lfs_file_seek(lfs_t *lfs, lfs_file_t *file,
    ...
    fvdbg("lfs_file_seek(%p, %p, %d, %d)",
            (void*)lfs, (void*)file, off, whence);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file));

    lfs_soff_t res = lfs_file_rawseek(lfs, file, off, whence);

    fvdbg("lfs_file_seek -> %d", res);
    LFS_UNLOCK(lfs->cfg);
    return res;
}
...
int lfs_file_truncate(lfs_t *lfs, lfs_file_t *file, lfs_off_t size) {
    ...
    fvdbg("lfs_file_truncate(%p, %p, %"PRIu32")",
            (void*)lfs, (void*)file, size);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file));

    err = lfs_file_rawtruncate(lfs, file, size);

    fvdbg("lfs_file_truncate -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
#endif
...
lfs_soff_t lfs_file_tell(lfs_t *lfs, lfs_file_t *file) {
    ...
    fvdbg("lfs_file_tell(%p, %p)", (void*)lfs, (void*)file);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file));

    lfs_soff_t res = lfs_file_rawtell(lfs, file);

    fvdbg("lfs_file_tell -> %d", res);
    LFS_UNLOCK(lfs->cfg);
    return res;
}
...
int lfs_file_rewind(lfs_t *lfs, lfs_file_t *file) {
    ...
    fvdbg("lfs_file_rewind(%p, %p)", (void*)lfs, (void*)file);

    err = lfs_file_rawrewind(lfs, file);

    fvdbg("lfs_file_rewind -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
...
lfs_soff_t lfs_file_size(lfs_t *lfs, lfs_file_t *file) {
    ...
    fvdbg("lfs_file_size(%p, %p)", (void*)lfs, (void*)file);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file));

    lfs_soff_t res = lfs_file_rawsize(lfs, file);

    fvdbg("lfs_file_size -> %d", res);
    LFS_UNLOCK(lfs->cfg);
    return res;
}
...
int lfs_mkdir(lfs_t *lfs, const char *path) {
    ...
    fvdbg("lfs_mkdir(%p, \"%s\")", (void*)lfs, path);

    err = lfs_rawmkdir(lfs, path);

    fvdbg("lfs_mkdir -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
#endif
...
int lfs_dir_open(lfs_t *lfs, lfs_dir_t *dir, const char *path) {
    ...
    fvdbg("lfs_dir_open(%p, %p, \"%s\")", (void*)lfs, (void*)dir, path);
    LFS_ASSERT(!lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)dir));

    err = lfs_dir_rawopen(lfs, dir, path);

    fvdbg("lfs_dir_open -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
...
int lfs_dir_close(lfs_t *lfs, lfs_dir_t *dir) {
    ...
    fvdbg("lfs_dir_close(%p, %p)", (void*)lfs, (void*)dir);

    err = lfs_dir_rawclose(lfs, dir);

    fvdbg("lfs_dir_close -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
...
int lfs_dir_read(lfs_t *lfs, lfs_dir_t *dir, struct lfs_info *info) {
    ...
    fvdbg("lfs_dir_read(%p, %p, %p)",
            (void*)lfs, (void*)dir, (void*)info);

    err = lfs_dir_rawread(lfs, dir, info);

    fvdbg("lfs_dir_read -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
...
int lfs_dir_seek(lfs_t *lfs, lfs_dir_t *dir, lfs_off_t off) {
    ...
    fvdbg("lfs_dir_seek(%p, %p, %"PRIu32")",
            (void*)lfs, (void*)dir, off);

    err = lfs_dir_rawseek(lfs, dir, off);

    fvdbg("lfs_dir_seek -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
...
lfs_soff_t lfs_dir_tell(lfs_t *lfs, lfs_dir_t *dir) {
    ...
    fvdbg("lfs_dir_tell(%p, %p)", (void*)lfs, (void*)dir);

    lfs_soff_t res = lfs_dir_rawtell(lfs, dir);

    fvdbg("lfs_dir_tell -> %d", res);
    LFS_UNLOCK(lfs->cfg);
    return res;
}
...
int lfs_dir_rewind(lfs_t *lfs, lfs_dir_t *dir) {
    ...
    fvdbg("lfs_dir_rewind(%p, %p)", (void*)lfs, (void*)dir);

    err = lfs_dir_rawrewind(lfs, dir);

    fvdbg("lfs_dir_rewind -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
...
lfs_ssize_t lfs_fs_size(lfs_t *lfs) {
    ...
    fvdbg("lfs_fs_size(%p)", (void*)lfs);

    lfs_ssize_t res = lfs_fs_rawsize(lfs);

    fvdbg("lfs_fs_size -> %d", res);
    LFS_UNLOCK(lfs->cfg);
    return res;
}
...
int lfs_fs_traverse(lfs_t *lfs, int (*cb)(void *, lfs_block_t), void *data) {
    ...
    fvdbg("lfs_fs_traverse(%p, %p, %p)",
            (void*)lfs, (void*)(uintptr_t)cb, data);

    err = lfs_fs_rawtraverse(lfs, cb, data, true);

    fvdbg("lfs_fs_traverse -> %d", err);
    LFS_UNLOCK(lfs->cfg);
    return err;
}
...
int lfs_migrate(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    fvdbg("lfs_migrate(%p, %p {.context=%p, "
                ".read=%p, .prog=%p, .erase=%p, .sync=%p, "
                ".read_size=%"PRIu32", .prog_size=%"PRIu32", "
                ".block_size=%"PRIu32", .block_count=%"PRIu32", "
                ".block_cycles=%"PRIu32", .cache_size=%"PRIu32", "
